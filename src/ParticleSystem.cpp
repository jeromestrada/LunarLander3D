...
// Impulse Directional Force - this is a "one shot" force that
// eminates towards a given direction.
//
ImpulseDirectionalForce::ImpulseDirectionalForce(float magnitude, ofVec3f direction) {
	this->magnitude = magnitude;
...
void ImpulseDirectionalForce::updateForce(Particle * particle) {

	// we basically create a random direction for each particle
	// the force is only added once after it is triggered.
	//
	particle->forces += direction.getNormalized() * magnitude;
}
